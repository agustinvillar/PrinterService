...
namespace POSPrinterService
{
    public partial class Service1 : ServiceBase
    {
        ...
        public void GetOrders()
        {
            ...
            db.Collection("orderFamily").WhereEqualTo("storeId", storeId).OrderByDescending("createdAt").Limit(1).Listen(snapshot =>
            {
                if (snapshot.Count > 0)
                {
        ...
        private static void pd_PrintPage(object sender, PrintPageEventArgs ev)
        {
            ...
            {
                text += orden.Items[i].Name + "x" + orden.Items[i].Quantity + " $" + (orden.Items[i].Price * orden.Items[i].Quantity) + "\n";
            }
            if (orden.OrderType == "Mesas")
            {
                float yPos;
                float xPos = 35;
                int count = 0;
